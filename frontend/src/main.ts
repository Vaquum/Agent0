import {
  fetchRunning, fetchQueued, fetchHistory, fetchLogs, fetchExecutorOutput, fetchHistoryOutput,
  type RunningTask, type QueuedTask, type HistoryEntry, type LogEntry,
} from './api';
...
let cachedRunning: RunningTask[] = [];
let cachedHistory: HistoryEntry[] = [];
let lastRunningFetch = 0;

let viewMode: 'log' | 'executor' | 'history' = 'log';
...
function switchToExecutor(repo: string): void {
  viewMode = 'executor';
  selectedRepo = repo;
  selectedNotificationId = null;
  selectedTimestamp = null;
...
function switchToHistory(
  notificationId: string, repo: string, timestamp: string,
): void {
  viewMode = 'history';
  selectedRepo = repo;
...
function updatePanelHeader(): void {
...
function renderRunning(tasks: RunningTask[]): void {
  cachedRunning = tasks;
...
function tickElapsed(): void {
  if (cachedRunning.length === 0) return;
  const drift = (Date.now() - lastRunningFetch) / 1000;
  const cells = document.querySelectorAll<HTMLElement>('.elapsed-cell');
  cells.forEach(cell => {
...
function renderQueued(tasks: QueuedTask[]): void {
...
function renderHistory(entries: HistoryEntry[]): void {
  ...
  document.getElementById('prev-page')?.addEventListener('click', () => {
    if (currentPage > 1) { currentPage--; refreshTasks(); }
  });
  document.getElementById('next-page')?.addEventListener('click', () => {
    if (entries.length >= 50) { currentPage++; refreshTasks(); }
  });
}
...
function appendOutputLine(text: string): void {
  const container = document.getElementById('log-output')!;
  const line = document.createElement('div');
...
async function refreshExecutorOutput(): Promise<void> {
...
async function refreshHistoryOutput(): Promise<void> {
  if (!selectedNotificationId || !selectedTimestamp) return;
  try {
    const data = await fetchHistoryOutput(
      selectedNotificationId, selectedTimestamp,
    );
    for (const entry of data.entries) {
      appendOutputLine(entry.text);
    }
...
async function refreshRightPanel(): Promise<void> {
  if (viewMode === 'executor' && selectedRepo) {
...
function initLogLevelFilter(): void {
  const select = document.getElementById('log-level') as HTMLSelectElement | null;
  if (!select) return;
