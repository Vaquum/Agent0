...
class TestPromptsModule:
    ...
    def test_all_templates_format_without_error(self) -> None:
        ...
        prompts.PREAMBLE.format(
            owner='testorg', repo='testrepo', whitelisted_orgs='testorg',
        )
        prompts.MENTION_ISSUE.format(
            number=1, title='test', issue_body='body',
            formatted_comments='comments', trigger_text='text',
        )
        prompts.MENTION_PR.format(
            number=1, title='test', pr_body='body', diff='diff',
            formatted_comments='comments', trigger_text='text',
        )
        prompts.ASSIGNED_ISSUE.format(
            number=1, title='test', issue_body='body',
            labels='bug', formatted_comments='comments',
        )
        prompts.REVIEW_PR.format(
            number=1, title='test', pr_body='body',
            head_ref='feature', base_ref='main', diff='diff',
            formatted_comments='comments', owner='testorg',
            repo='testrepo', github_user='zero-bang',
        )
        prompts.CI_FAILURE.format(
            number=1, title='test', head_ref='feature', base_ref='main',
            check_failures='failures', diff='diff', formatted_comments='comments',
        )
